...
#define k_threshold 1000 //TODO: experiment with this value..
...
class Component
{
	public:
		...
		void mergeComponent(Component c2, double weight)
		{
			double w=weight;
			if(w>c2.maxEdgeWeight)
			{
				w=c2.maxEdgeWeight;
			}
			merge(c2.liNodes, w);
...
void segmentation(CImg<double> &inp, bool isSimpleSeg, bool removeBack)
{
	...
	{	
		...
		{   
			...
			it1->mergeComponent(*it2,liEdges[i].weight);
...
int main(int argc, char **argv) 
{
	...
	{		
		input.resize(-20, -20, -100, -100, 5);
		cout << "# Problem 3.1 - Finding Circular Regions (Simple Segmentation Algorithm)" << endl;				
	...
	{		
		...
		cout << "# Problem 3.2 - Finding Circular Regions (Modified Segmentation Algorithm)" << endl;				
